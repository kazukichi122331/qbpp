...
> **続きがある**: このレポートのあと、同じ日に `src/` の中身も整理した。
> ファイル名（`tsptw.py` → `order_cumulative.py` など）と図のファイル名、
> 実行方法が本レポートの記述から変わっている。
> 対応表は [docs/src_refactor_20260910.md](src_refactor_20260910.md) を参照。
