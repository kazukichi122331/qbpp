...
from archive.tsptw.archive.travel_time import travel_time_double
from archive.tsptw.archive.tsptw_plot import plot_tour
...
c = []
for u in range(N+1):
    c.append([travel_time_double(u, v, nodes) for v in range(N+1)])
...
f = objective + (tour_P*tour_constraints + tw_P*tw_constraints)
...
print(f"energy = {sol(g):.2f}")
print(f"objective = {sol(objective):.2f}")
print(f"constraint = {sol(tour_constraints)+sol(tw_constraints):.2f}")
...
print(f"time_margin_constraint = {sol(time_margin_constraint):.2f}")
print("K = ", K)

tour = make_tour(sol)
print(tour)

filename = "tsptw_d_" + datetime.now().strftime("%m%d%H%M")
...
for i in range(1, N+1):
    for u in range(N+1):
        for v in range(N+1):
            if sol(x[u][v][i]) == 1:
                arrival_times[v] = round(sol(a[i]), 2)
