...
from archive.tsptw.archive.tsptw_leq.travel_time import travel_time
from archive.tsptw.archive.tsptw_leq.tsptw_plot import plot_tour
from archive.tsptw.archive.tsptw_leq.time_nodes import time_nodes_10, time_nodes_15, time_nodes_20
...
t = [0] #i番目までの合計時間
for i in range(1, N+1):
    next_t = a[i] + sum(w[j] for j in range(1, i))
    t.append(next_t)
...
time_constraints = []
for i in range(N + 1):
    service_start = t[i] + w[i]
    time_constraints.append(service_start - e[i] >= 0)
    time_constraints.append(service_start - l[i] <= 0)
